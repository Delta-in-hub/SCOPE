...
static inline const char *parse_syscall(const char *buf, int *number) {
    char *end;
    long x;
...
static inline void init_syscall_names(void) {
    ...
    syscall_names = (const char **)calloc(size, sizeof(char *));
    if (!syscall_names) {
        warn("calloc: %s\n", strerror(errno));
        goto close;
    }
    ...
    while (fgets(buf, sizeof(buf), f)) {
        ...
        if (number >= size) {
            ...
            syscall_names =
                (const char **)realloc(syscall_names, size * sizeof(char *));
            if (!syscall_names) {
                warn("realloc: %s\n", strerror(errno));
                goto close;
            }
        ...
        if (syscall_names[number]) {
            warn("duplicate number: %d (stored: %s)", number,
...
close:
    ...
    /* Ignore the error for x86_64/arm64/riscv/loongarch64 where we have a table
     * compiled in */
...
static inline void free_syscall_names(void) {
    size_t i;
...
size_t syscall_names_x86_64_size =
...
size_t syscall_names_generic_size =
    sizeof(syscall_names_generic) / sizeof(char *);
#endif

static inline void syscall_name(unsigned n, char *buf, size_t size) {
...
static inline int list_syscalls(void) {
