...
const char argp_program_doc[] =
    ...
    "Traces cudaMalloc, cudaFree, cudaLaunchKernel, cudaMemcpy, "
...
static const struct argp_option opts[] = {
    ...
    {"file", 'f', "FILE_PATH", 0,
     "Path to the target libcudart.so (default: " DEFAULT_CUDA_LIB_PATH ")"},
...
static const char *memcpy_kind_to_str(enum cuda_memcpy_kind kind) {
    switch (kind) {
    case CUDA_MEMCPY_HOST_TO_HOST:
        return "HostToHost";
    case CUDA_MEMCPY_HOST_TO_DEVICE:
        return "HostToDevice";
    case CUDA_MEMCPY_DEVICE_TO_HOST:
        return "DeviceToHost";
    case CUDA_MEMCPY_DEVICE_TO_DEVICE:
        return "DeviceToDevice";
    case CUDA_MEMCPY_DEFAULT:
        return "Default";
    default:
        return "Unknown";
    }
...
static int handle_event(void *ctx, void *data, size_t data_sz) {
    ...
    if (env.verbose) {
        ...
        case EVENT_TYPE_MEMCPY:
            ...
            printf("cudaMemcpy %s(src=%p, dst=%p, size=%zu)\n",
                   memcpy_kind_to_str(e->memcpy.kind), e->memcpy.src,
                   e->memcpy.dst, e->memcpy.size);
            break;
        case EVENT_TYPE_SYNC:
            // "%d %s call cudaDeviceSynchronize at %u\n" (bpftrace used nsecs,
            // let's just mark entry)
    ...
    if (e->type == EVENT_TYPE_MALLOC) {
        struct cuda_malloc_event event = {
            .timestamp_ns = UnixNanoNow(),
            .pid = e->pid,
            .comm = "",
            .allocated_ptr = (uint64_t)e->malloc.allocated_ptr,
            .size = e->malloc.size,
            .retval = e->malloc.retval,
        };
        strncpy(event.comm, e->comm, sizeof(event.comm));
        zmq_pub_send(zmq_handle, "cudaMalloc", &event, cuda_malloc_event_pack);
    } else if (e->type == EVENT_TYPE_FREE) {
        struct cuda_free_event event = {
            .timestamp_ns = UnixNanoNow(),
            .pid = e->pid,
            .comm = "",
            .dev_ptr = (uint64_t)e->free.dev_ptr,
        };
        strncpy(event.comm, e->comm, sizeof(event.comm));
        zmq_pub_send(zmq_handle, "cudaFree", &event, cuda_free_event_pack);
    } else if (e->type == EVENT_TYPE_LAUNCH_KERNEL) {
        struct cuda_launch_kernel_event event = {
            .timestamp_ns = UnixNanoNow(),
            .pid = e->pid,
            .comm = "",
            .func_ptr = (uint64_t)e->launch_kernel.func_ptr,
        };
        strncpy(event.comm, e->comm, sizeof(event.comm));
        zmq_pub_send(zmq_handle, "cudaLaunchKernel", &event,
                     cuda_launch_kernel_event_pack);
    } else if (e->type == EVENT_TYPE_MEMCPY) {
        struct cuda_memcpy_event event = {
            .timestamp_ns = UnixNanoNow(),
            .pid = e->pid,
            .comm = "",
            .src = (uint64_t)e->memcpy.src,
            .dst = (uint64_t)e->memcpy.dst,
            .size = e->memcpy.size,
            .kind = e->memcpy.kind,
        };
        strncpy(event.comm, e->comm, sizeof(event.comm));
        zmq_pub_send(zmq_handle, "cudaMemcpy", &event, cuda_memcpy_event_pack);
    } else if (e->type == EVENT_TYPE_SYNC) {
        struct cuda_sync_event event = {
            .timestamp_ns = UnixNanoNow(),
            .pid = e->pid,
            .comm = "",
            .duration_ns = e->sync.duration_ns,
        };
        strncpy(event.comm, e->comm, sizeof(event.comm));
        zmq_pub_send(zmq_handle, "cudaDeviceSynchronize", &event,
                     cuda_sync_event_pack);
    } else {
        fprintf(stderr, "Warning: Unknown event type received: %d\n", e->type);
    }
...
int main(int argc, char **argv) {
    ...
    zmq_pub_handle_t *ctx = zmq_pub_init(ENDPOINT);
...
#define ATTACH_UPROBE_URETPROBE(skel, sym, prog_uprobe_name,                   \
                                prog_uretprobe_name)                           \
    do {                                                                       \
        /* Attach Uprobe */                                                    \
        ...
        skel->links.prog_uprobe_name =                                         \
            bpf_program__attach_uprobe_opts(skel->progs.prog_uprobe_name, -1,  \
                                            env.target_path, 0, &uprobe_opts); \
        ...
        skel->links.prog_uretprobe_name = bpf_program__attach_uprobe_opts(     \
            skel->progs.prog_uretprobe_name, -1, env.target_path, 0,           \
            &uprobe_opts);                                                     \
...
#define ATTACH_UPROBE(skel, sym, prog_uprobe_name)                             \
    ...
    ATTACH_UPROBE_URETPROBE(skel, TARGET_FUNC_MALLOC, uprobe_cudaMalloc,
    ...
    // Pass both the uprobe and uretprobe member names
    ATTACH_UPROBE_URETPROBE(skel, TARGET_FUNC_SYNC,
                            uprobe_cudaDeviceSynchronize,
                            uretprobe_cudaDeviceSynchronize);
