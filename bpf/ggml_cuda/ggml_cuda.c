...
#define TARGET_FUNC_MUL_MAT_VEC_Q "_Z26ggml_cuda_op_mul_mat_vec_qR25ggml_backend_cuda_contextPK11ggml_tensorS3_PS1_PKcPKfS6_PfllllP11CUstream_st"
#define TARGET_FUNC_MUL_MAT_Q "_Z22ggml_cuda_op_mul_mat_qR25ggml_backend_cuda_contextPK11ggml_tensorS3_PS1_PKcPKfS6_PfllllP11CUstream_st"
...
static const struct argp_option opts[] = {
    ...
    {"file", 'f', "FILE_PATH", 0, "Path to the target libggml-cuda.so (default: " DEFAULT_TARGET_LIB ")"},
...
static int handle_event(void *ctx, void *data, size_t data_sz) {
    if (exiting) return -1; // 如果正在退出，停止处理
    ...
    case EVENT_TYPE_FUNC_DURATION:
        printf("%-8s %-7d %-16s FUNC %-30s cost %lu ns\n",
               ts, e->pid, e->comm, e->func_duration.func_name, e->func_duration.duration_ns);
        break;
    case EVENT_TYPE_SET_DEVICE:
        printf("%-8s %-7d %-16s CALL ggml_cuda_set_device(%d)\n",
               ts, e->pid, e->comm, e->set_device.device_id);
        break;
    default:
...
int main(int argc, char **argv) {
    ...
    // BPF 端 filter_comm 已经初始化为全零，strncpy 会复制 null 终止符 (如果源字符串长度小于 TASK_COMM_LEN)
    ...
    skel->links.uprobe_ggml_cuda_op_mul_mat_vec_q = bpf_program__attach_uprobe_opts(
    ...
    skel->links.uretprobe_ggml_cuda_op_mul_mat_vec_q = bpf_program__attach_uprobe_opts(
    ...
    skel->links.uprobe_ggml_cuda_op_mul_mat_q = bpf_program__attach_uprobe_opts(
        skel->progs.uprobe_ggml_cuda_op_mul_mat_q, -1, env.target_path, 0, &uprobe_opts);
    ...
    skel->links.uretprobe_ggml_cuda_op_mul_mat_q = bpf_program__attach_uprobe_opts(
        skel->progs.uretprobe_ggml_cuda_op_mul_mat_q, -1, env.target_path, 0, &uprobe_opts);
    ...
    // uprobe_opts.retprobe = false;
    // skel->links.uprobe_ggml_cuda_set_device = bpf_program__attach_uprobe_opts(
    //     skel->progs.uprobe_ggml_cuda_set_device, -1, env.target_path, 0, &uprobe_opts);
    // if (!skel->links.uprobe_ggml_cuda_set_device) {
    //     err = -errno;
    //     fprintf(stderr, "Failed to attach uprobe %s: %s\n", TARGET_FUNC_SET_DEVICE, strerror(-err));
    //     goto cleanup;
    // }
...
cleanup:
    ...
    // ggml_cuda_bpf__destroy 会自动 detach 通过 skel->links 附加的探针并释放所有 BPF 对象
