...
void pack_status(msgpack_packer *pk, const StatusUpdatePayload *payload) {
  ...
  size_t code_len = strnlen(payload->status_code, sizeof(payload->status_code));
...
int send_multipart_topic_msg(void *socket, const char *topic_str,
  ...
  zmq_msg_close(&topic_msg); // Always close the message after sending or error
  if (rc == -1) {
...
int main() {
  ...
  for (int i = 0; i < 10; ++i) {
    ...
    if (i % 2 == 0) {
      ...
      // Pack the command payload into the reusable buffer (sbuf) via the packer
      // (pk)
      pack_command(&pk, &cmd);
      ...
      stat.details[sizeof(stat.details) - 1] = '\0'; // Ensure null termination

      // Pack the status payload into the reusable buffer (sbuf) via the packer
      // (pk)
      pack_status(&pk, &stat);

      // Print limited details to keep output concise
      printf("Publishing [%s]: SrcID=%d, Code='%s', Details='%.30s...' ", topic,
             stat.source_id, stat.status_code, stat.details);
    }

    // At this point, 'sbuf' contains the packed data for the current message
    ...
    // This resets sbuf.size to 0 but keeps the allocated memory (sbuf.data)
    // ready for reuse, avoiding repeated malloc/free.
