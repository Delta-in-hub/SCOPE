...
const char argp_program_doc[] =
    "ggml_base: Monitor ggml_aligned_malloc/free calls in a shared library using eBPF.\n\n"
    "USAGE: ./ggml_base [-p PID] [-c COMM] [-l LIBRARY_PATH] [-v]\n";

// 命令行选项定义
static const struct argp_option opts[] = {
    {"pid", 'p', "PID", 0, "Filter by process PID"},
    {"comm", 'c', "COMMAND", 0, "Filter by process command name (max 15 chars)"},
    {"lib", 'l', "LIBRARY_PATH", 0, "Path to the target shared library (default: " DEFAULT_TARGET_LIB ")"},
...
static int handle_event(void *ctx, void *data, size_t data_sz) {
    if (exiting) return -1; // 收到退出信号后停止处理
    ...
    printf("%-8s %-7u %-16s %-8s %-10llu 0x%llx\n",
           ts, e->pid, e->comm, type_str, e->size, e->ptr);
...
int main(int argc, char **argv) {
    ...
    skel->links.uprobe_ggml_aligned_malloc = bpf_program__attach_uprobe_opts(
                                      skel->progs.uprobe_ggml_aligned_malloc,
                                      -1,                  // pid: -1 表示附加到所有进程
    ...
    skel->links.uretprobe_ggml_aligned_malloc = bpf_program__attach_uprobe_opts(
                                      skel->progs.uretprobe_ggml_aligned_malloc,
                                      -1,
                                      env.target_lib,
                                      0,
                                      &uprobe_opts);
    ...
    uprobe_opts.retprobe = false; // 这是入口探针
    skel->links.uprobe_ggml_aligned_free = bpf_program__attach_uprobe_opts(
                                      skel->progs.uprobe_ggml_aligned_free,
                                      -1,
                                      env.target_lib,
                                      0,
                                      &uprobe_opts);
    ...
    // --- 事件轮询循环 ---
    printf("Monitoring ggml memory operations in %s (Press Ctrl+C to exit)...\n", env.target_lib);
