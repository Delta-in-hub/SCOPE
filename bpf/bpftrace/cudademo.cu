...
inline void checkCudaError(cudaError_t err, const char *msg) {
    if (err != cudaSuccess) {
        std::cerr << "CUDA错误: " << msg << " (" << cudaGetErrorString(err)
                  << ")" << std::endl;
        exit(EXIT_FAILURE);
    }
}
...
int main() {
    ...
    std::cout << "CUDA计算能力: " << prop.major << "." << prop.minor
              << std::endl;

    // 数组元素数量
    const int numElements = 50000;
    std::cout << "数组相加示例，每个数组包含 " << numElements << " 个元素"
              << std::endl;
    ...
    // 分配主机内存（使用固定内存提高性能）
    float *h_A, *h_B, *h_C;
    checkCudaError(cudaMallocHost(&h_A, numElements * sizeof(float)),
                   "分配固定主机内存A失败");
    checkCudaError(cudaMallocHost(&h_B, numElements * sizeof(float)),
                   "分配固定主机内存B失败");
    checkCudaError(cudaMallocHost(&h_C, numElements * sizeof(float)),
                   "分配固定主机内存C失败");
    ...
    // 分配设备内存
    float *d_A, *d_B, *d_C;
    checkCudaError(cudaMalloc(&d_A, numElements * sizeof(float)),
                   "分配设备内存A失败");
    checkCudaError(cudaMalloc(&d_B, numElements * sizeof(float)),
                   "分配设备内存B失败");
    checkCudaError(cudaMalloc(&d_C, numElements * sizeof(float)),
                   "分配设备内存C失败");

    // 将数据从主机复制到设备
    std::cout << "将数据从主机内存复制到设备内存..." << std::endl;
    checkCudaError(cudaMemcpy(d_A, h_A, numElements * sizeof(float),
                              cudaMemcpyHostToDevice),
                   "复制数据A到设备失败");
    checkCudaError(cudaMemcpy(d_B, h_B, numElements * sizeof(float),
                              cudaMemcpyHostToDevice),
                   "复制数据B到设备失败");
    ...
    checkCudaError(cudaMemcpy(h_C, d_C, numElements * sizeof(float),
